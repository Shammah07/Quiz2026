...
export function JudgeDashboard({ config, teams, scores, judgeName, onRegisterTeam, onScoresChange, onRefresh, onLogout }) {
    ...
    <div className="space-y-2">
      ...
      {rows.map((p) => (
        <div key={p.id}>
          ...
          {p.viaTiebreak && (
            <div className="text-[10px] mt-0.5 text-center" style={{ color: "#0F8A6B" }}>
              Decided by Category of Choice tiebreaker
            </div>
          )}
        </div>
      ))}
