...
/* ---------------------------------------------------------
   TEAMS — a real table, not a blob.
   Unlike config/scores/pairings, teams need a hard duplicate-name
   guarantee even when two judges submit at the same moment, and
   need to stay visible to every judge/admin without one submission
   silently overwriting another's. A shared JSON array can't give
   either guarantee, so teams live in their own Postgres table with
...
export async function registerTeam({ tournamentId, name, category, members, judgeName }) {
  ...
  if (error) {
    ...
    console.error("Failed to register team:", error.message);
    return { ok: false, error: "Could not register the team. Try again." };
  }
  return { ok: true };
}
...
export function roundsForPairings(config, pairings, releasedOnly = false) {
  const roundIds = new Set();
  pairings
    .filter((pairing) => !releasedOnly || pairing.released === true)
    .forEach((pairing) => {
      stageRoundsFor(config, pairing.stage, pairing.matchLabel).forEach((round) => roundIds.add(round.id));
    });
...
// that match's own rounds. If tied, defers to a recorded Category of
// Choice tiebreak result (pairing.tiebreakWinnerId) rather than
// guessing — a genuine tie needs that manual tiebreaker, not an
// automatic call.
export function matchWinnerDetail(pairing, config, scores) {
  if (!pairing.teamAId || !pairing.teamBId) {
    return { winnerId: pairing.teamAId || pairing.teamBId || null, viaTiebreak: false }; // bye
  }
  ...
  if (a === b) {
    return { winnerId: pairing.tiebreakWinnerId || null, viaTiebreak: !!pairing.tiebreakWinnerId };
  }
  return { winnerId: a > b ? pairing.teamAId : pairing.teamBId, viaTiebreak: false };
}

export function matchWinner(pairing, config, scores) {
  return matchWinnerDetail(pairing, config, scores).winnerId;
}

// True when a knockout match is tied on raw points, at least one
// score has been entered for it, and no Category of Choice result
// has been recorded yet — this is exactly when the admin UI should
// prompt for the tiebreaker.
export function isTiedNeedingBreak(pairing, config, scores) {
  if (!pairing.teamAId || !pairing.teamBId || pairing.tiebreakWinnerId) return false;
  const rounds = stageRoundsFor(config, pairing.stage, pairing.matchLabel);
  const ids = new Set(rounds.map((r) => r.id));
  const hasAnyScore = scores.some((s) => (s.teamId === pairing.teamAId || s.teamId === pairing.teamBId) && ids.has(s.roundId));
...
// Requires a second tap within 5 seconds before it actually fires,
// so a stray click can't wipe tournament data. Only ever rendered
// inside AdminDashboard, so only a signed-in admin can reach it.
function DangerZone({ onResetTournament }) {
...
export function AdminDashboard({ config, teams, scores, pairings, rooms, judges, tournaments, activeTournamentId, onConfigChange, onDeleteTeam, onScoresChange, onPairingsChange, onRoomsChange, onJudgesChange, onCreateTournament, onSelectTournament, onResetTournament, onRefresh, onLogout }) {
  ...
  // Live sync: admin sees teams/scores registered by judges, and
  // pairing winners, without needing to tap the manual refresh icon.
  useEffect(() => {
    ...
    <div>
      ...
      {tab === "overview" && (
        <div>
          ...
          <DangerZone onResetTournament={onResetTournament} />
        </div>
      )}
...
// Double-tap confirm, matching DangerZone, but scoped to just this
// draw's pairings rather than the whole tournament.
function ResetDrawButton({ onConfirm }) {
  const [confirming, setConfirming] = useState(false);
  const timerRef = useRef(null);
  ...
  return (
    <Btn variant={confirming ? "danger" : "ghost"} onClick={handleClick} className="w-full">
      <Trash2 size={14} /> {confirming ? "Click again to clear the whole draw" : "Reset draw"}
    </Btn>
  );
}
...
export function PairingsPanel({ config, teams, scores, pairings, rooms, judges, onPairingsChange }) {
  ...
  const setTiebreak = async (pairingId, winnerId) => {
    await onPairingsChange(pairings.map((p) => (p.id === pairingId ? { ...p, tiebreakWinnerId: winnerId } : p)));
  };
  const clearTiebreak = async (pairingId) => {
    await onPairingsChange(pairings.map((p) => (p.id === pairingId ? { ...p, tiebreakWinnerId: null } : p)));
  };
  const setReleased = async (stage, matchLabel, released) => {
    await onPairingsChange(pairings.map((p) => (
      p.stage === stage && (stage !== "preliminary" || p.matchLabel === matchLabel)
        ? { ...p, released }
        : p
    )));
  };
  ...
  return (
    <div className="space-y-3">
      {pairings.length > 0 && (
        <div className="rounded-xl border p-4" style={{ borderColor: "#F3C8C6", background: "#FDECEB" }}>
          <div className="font-semibold text-sm mb-1" style={{ color: "#C1443E" }}>Drew the wrong pairings?</div>
          <p className="text-xs mb-3" style={{ color: "#C1443E" }}>
            Clears every generated pairing across all stages so you can redraw from scratch. Team registrations and scores already entered are kept.
          </p>
          <ResetDrawButton onConfirm={() => onPairingsChange([])} />
        </div>
      )}
        ...
        return (
          <div key={sec.label} className="rounded-xl border p-4" style={{ borderColor: "#DBD8CE", background: "#FFFFFF" }}>
            ...
            {!sec.ready ? (
              ...
              <div className="space-y-2">
                {rows.map((p) => {
                  const isKnockout = sec.stage !== "preliminary";
                  const detail = isKnockout ? matchWinnerDetail(p, config, scores) : { winnerId: null, viaTiebreak: false };
                    ...
                    <div key={p.id} className="rounded-lg px-2.5 py-1.5" style={{ background: "#F7F5F0" }}>
                      ...
                      {detail.viaTiebreak && (
                        <div className="flex items-center justify-between mt-1.5">
                          ...
                          <button onClick={() => clearTiebreak(p.id)} className="text-[10px] underline" style={{ color: "#9098B0" }}>
                            Clear
                          </button>
                        </div>
                      )}
                      {tied && (
                        <div className="mt-2 rounded-lg px-2.5 py-2" style={{ background: "#FDECEB" }}>
                          <div className="text-[11px] font-semibold mb-1.5" style={{ color: "#C1443E" }}>
                            Tied on points — run the Category of Choice tiebreaker, then record the winner
                          </div>
                          <div className="flex gap-2">
                            <Btn variant="ghost" className="flex-1 text-xs" onClick={() => setTiebreak(p.id, p.teamAId)}>
                              {teamName(p.teamAId)} wins
                            </Btn>
                            <Btn variant="ghost" className="flex-1 text-xs" onClick={() => setTiebreak(p.id, p.teamBId)}>
                              {teamName(p.teamBId)} wins
                            </Btn>
                          </div>
                        </div>
                      )}
...
export function JudgeDashboard({ config, teams, scores, pairings, judgeName, onRegisterTeam, onScoresChange, onRefresh, onLogout }) {
  ...
  // Live sync: this judge sees teams other judges just registered,
  // and current scores, without needing a manual refresh.
  useEffect(() => {
    const interval = setInterval(() => { onRefresh(); }, 6000);
    return () => clearInterval(interval);
  }, [onRefresh]);
  ...
  return (
    <div>
      ...
      />
      {tab === "register" && (
        <RegisterTeamForm teams={teams} onRegisterTeam={onRegisterTeam} />
      )}
      {tab === "score" && (
        <EnterScoreForm config={config} teams={teams} scores={scores} pairings={pairings} judgeName={judgeName} onScoresChange={onScoresChange} />
      )}
      {tab === "teams" && (
        <TeamsPanel teams={teams} editable={false} />
      )}
    </div>
  );
}

export function RegisterTeamForm({ teams, onRegisterTeam }) {
  const [name, setName] = useState("");
  ...
  const submit = async (e) => {
    ...
    // Instant local check for a fast "no" — the database's own
    // unique constraint is what actually prevents a duplicate if two
    // judges submit at nearly the same moment.
    if (teams.some((t) => t.name.toLowerCase() === name.trim().toLowerCase())) {
      return setErr("A team with that name already exists.");
    }
    setErr("");
    setSubmitting(true);
    const result = await onRegisterTeam({ name: name.trim(), category: category.trim(), members: members.trim() });
    setSubmitting(false);
    if (!result.ok) {
      setErr(result.error || "Could not register the team.");
      return;
    }
  ...
  return (
    <form onSubmit={submit}>
      ...
      <Btn variant="accent" type="submit" className="w-full" disabled={submitting}>
        <Plus size={16} /> {submitting ? "Registering…" : "Register team"}
      </Btn>
    </form>
  );
}
...
export function EnterScoreForm({ config, teams, scores, pairings, judgeName, onScoresChange }) {
  ...
  const [err, setErr] = useState("");
  const [saved, setSaved] = useState(false);
  const activeRoundId = generatedRounds.some((round) => round.id === roundId) ? roundId : generatedRounds[0]?.id || "";

  const existing = scores.find((s) => s.teamId === teamId && s.roundId === activeRoundId);

  const submit = async (e) => {
    e.preventDefault();
    if (!teamId) return setErr("Choose a team.");
    if (!activeRoundId) return setErr("Choose a round.");
  ...
  return (
    <div>
      {teams.length === 0 ? (
        ...
        <EmptyNote>The admin has not generated a round yet. Scores will unlock after the draw is generated.</EmptyNote>
      ) : (
        <form onSubmit={submit}>
          {err && <Banner>{err}</Banner>}
          ...
          <Field label="Round">
            <select
              ...
              style={{ borderColor: "#DBD8CE", background: "#FFFFFF", color: "#14213D" }}
            >
              {groupRoundsForDisplay(generatedRounds).map((g) => (
                <optgroup key={g.key} label={g.label}>
                  {g.items.map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}
                </optgroup>
              ))}
...
export function BracketPanel({ config, teams, scores, pairings }) {
  ...
  const stageRows = (stage) =>
    pairings
      .filter((p) => p.stage === stage && p.released)
      .map((p) => ({ ...p, ...matchWinnerDetail(p, config, scores) }));

  const preliminary = pairings
    .filter((p) => p.stage === "preliminary" && p.released)
    .map((p) => ({ ...p, winnerId: null, viaTiebreak: false }));

  const qf = stageRows("quarter-final");
  const sf = stageRows("semi-final");
  const final = stageRows("final");

  if (qf.length === 0 && preliminary.length === 0) {
    return (
